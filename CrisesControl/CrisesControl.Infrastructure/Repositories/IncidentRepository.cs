...
public class IncidentRepository : IIncidentRepository {
    ...
    public async Task AddIncidentActivation(IncidentActivation incidentActivation, CancellationToken cancellationToken) {
        await _context.AddAsync(incidentActivation);
        await _context.SaveChangesAsync();
    }
    ...
    public async Task ProcessKeyHolders(int companyId, int incidentId, int currentUserId, int[] keyHolders) {
        var incKeyHolders = await _context.Set<IncidentKeyholder>()
    ...
    public async Task SaveIncidentMessageResponse(ICollection<AckOption> ackOptions, int incidentId) {
        ...
        _context.AddRange(incidentMessageToAdd);
    ...
    public async Task AddIncidentGroup(int incidentId, int[] groups, int companyId) {
        ...
        _context.AddRange(groupsToSave);
        await _context.SaveChangesAsync();
    }

    public void CreateIncidentSegLinks(int incidentId, int userId, int companyId) {
        var pCompanyId = new SqlParameter("@CompanyId", companyId);
        var pUserId = new SqlParameter("@UserId", userId);
        var pIncidentId = new SqlParameter("@IncidentId", incidentId);

        _context.Database.ExecuteSqlRaw("exec Pro_Incident_CreateLink @CompanyId, @UserId, @IncidentId", pCompanyId, pUserId, pIncidentId);
    }
    ...
    public async Task<ICollection<DataIncidentType>> CopyIncidentTypes(int userId, int companyId) {
        var libIncidentTypeData = _context.Set<LibIncidentType>().ToList();
    ...
    public async Task CopyIncidentToCompany(int companyId, int userId, string timeZoneId = "GMT Standard Time") {
        var sourceIncidentTypes = CopyIncidentTypes(userId, companyId).Result;
        ...
        foreach (var incident in incidentsFromLib) {
            ...
            if (!isIncidentExists) {
                ...
                if (incidentType != null) {
                    var keyHolderList =  await _context.Set<User>().Where(x => x.CompanyId == companyId && x.Status != 3)
                        .Select(x => new AddIncidentKeyHldLst(x.UserId)).ToListAsync();
                    ...
                    var incidentId = AddIncident(incidentNew).Result;
    ...
    public async Task<int> ActivateSampleIncident(int UserID, int CompanyID, string TimeZoneID) {
        try {
            ...
            if (check_incident == null) {
                IEnumerable<DataIncidentType> source_incident_type = CopyIncidentTypes(UserID, CompanyID).Result;
    ...
    public async Task<int> AddCompanyIncidents(
        ...
        Incident tblIncident = new Incident() {
            ...
            UpdatedOn = DateTimeExtensions.GetLocalTime(TimeZoneId, System.DateTime.Now),
        ...
        if (tblIncident.IncidentId > 0) {
            ...
            CreateIncidentSegLinks(tblIncident.IncidentId, CurrentUserId, CompanyId);
                ...
                foreach (var IKeyHld in LstKeyHld) {
                    if (IKeyHld.UserId != null) {
                        IncidentKeyContact tblIncKeyContact = new IncidentKeyContact() {
                            ...
                            CreatedOn = DateTime.Now,
                            UpdatedBy = UserID,
                            UpdatedOn = DateTimeExtensions.GetLocalTime(TimeZoneID, DateTime.Now)
                        };
    ...
    public async Task<int> CreateSOSIncident(int UserID, int CompanyID, string TimeZoneID) {
        try {
            ...
            var methods = await _context.Set<CompanyComm>().Where(CM => CM.CompanyId == CompanyID).Select(CM => CM.MethodId).ToListAsync();
            int[] Methods = methods.ToArray();
            ...
            //Attach the key contacts
            var KeyHolderList = _context.Set<User>().Where(U => U.CompanyId == CompanyID && U.Status != 3).Select(U => new AddIncidentKeyHldLst(U.UserId)).Take(2).ToArray();
    ...
    public async Task<UpdateIncidentStatusReturn> UpdateIncidentStatus(int CompanyId, int IncidentActivationId, string Type, string TimeZoneId, int CurrentUserId,
        ...
        if (Incidt != null) {
            ...
            if (Type.ToUpper() == "CLOSE") {
                ...
                int tblmessageid = _MSG.CreateMessage(CompanyId, MsgText, "Ping", IncidentActivationId, 500, CurrentUserId, 1, LocalTime, false, null, 99,
                    0, 0, false, false, MessageMethod).Result;
                ...
                try {
                    ...
                    _ = Task.Factory.StartNew(() => _queueHelper.MessageDeviceQueue(tblmessageid, "Ping", 1, CascadePlanID));
                ...
                await _MSG.DeleteMessageMethod(0, IncidentActivationId);
        ...
        try {
            ...
            if (UsersToNotify.Length > 0) {
                foreach (int userid in UsersToNotify) {
                    ...
                    if (ISExist == null) {
                        CreateIncidentParticipant(IncidentId, 0, 0, ActionID, userid, "USER");
                    } else {
                        int[] Arr = new int[3];
                        Arr[0] = ISExist.IncidentId;
        ...
        if (MessageMethod != null) {
            if (MessageMethod.Length > 0) {
                ...
                if (CascadePlanID <= 0) {
                    ...
                    foreach (int Method in MessageMethod) {
                        await _MSG.CreateMessageMethod(0, Method, 0, IncidentId);
                    }
                }
            }
        }
    ...
    public async Task<bool> DeleteIncidentAssets(int CompanyId, int IncidentId, int AssetObjMapId, int IncidentAssetId) {
        ...
        if (QueryRec != null) { //Do not delete all Action
    ...
    public async Task<UpdateIncidentStatusReturn> TestWithMe(int IncidentId, int[] ImpactedLocations, int CurrentUserId, int CompanyId, string TimeZoneId) {
        try {
            ...
            if (verifyInci != null) {
                ...
                int mPriority = Common.GetPriority(verifyInci.Severity);
                ...
                _MSG.CascadePlanID = verifyInci.CascadePlanId;
                _MSG.MessageSourceAction = SourceAction.IncidentTest;
                int tblmessageid = _MSG.CreateMessage(CompanyId, tblIncidenActivation.IncidentDescription, "Incident",
                ...
                await _MSG.AddUserToNotify(tblmessageid, new int[] { CurrentUserId }.ToList(), tblIncidenActivation.IncidentActivationId);
                ...
                try {
                    ...
                    IsFundAvailable = await _MSG.CalculateMessageCost(CompanyId, tblmessageid, tblIncidenActivation.IncidentDescription);
    ...
    public void CreateIncidentLocation(AffectedLocation loc, int ActiveIncidentID, int CompanyID) {
        try {
            ...
            _context.Database.ExecuteSqlRawAsync("Pro_Create_Incident_Location @ActiveIncidentID, @CompanyID, @LocationID, @ImpactedLocationID, @LocationName, @Lat, @Lng, @Address, @LocationType",
        ...
        if (inci != null) {
            ...
            if (MessageMethod != null) {
                if (MessageMethod.Length > 0) {
                    ...
                    foreach (int Method in MessageMethod) {
                        await _MSG.CreateMessageMethod(0, Method, inci.IncidentActivationId);
                        if (pushmethodid == Method)
                            pushadded = true;
                    }
                    if (TrackUser && !pushadded) {
                        await _MSG.CreateMessageMethod(0, pushmethodid, inci.IncidentActivationId);
                    }
                }
            }

            if (UsersToNotify != null) {
                await _MSG.AddUserToNotify(0, UsersToNotify.ToList(), inci.IncidentActivationId);
            }

            if (MultiResponse) {
                await _MSG.SaveActiveMessageResponse(0, AckOptions, inci.IncidentActivationId);
            }
    ...
    public void CleanUPJob(int ActiveIncidentID) {
        try {

            var pActiveIncidentID = new SqlParameter("@ActiveIncidentID", ActiveIncidentID);
            _context.Database.ExecuteSqlRawAsync("Pro_Job_Data_Cleanup @ActiveIncidentID", pActiveIncidentID);
