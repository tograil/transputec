    ...
    public class Messaging {
            ...
            try {
                ...
                if (messageMethod != null && CascadePlanID <= 0) {
                    ...
                    if (source == 3) {
                        ...
                        commsmethod = await (from MM in db.Set<MessageMethod>()
                    ...
                    try {
                        ...
                        foreach (MediaAttachment ma in mediaAttachments) {
                            ...
                            try {
                                ...
                                _ = CreateMediaAttachment(messageId, ma.Title, ma.FileName, ma.OriginalFileName, ma.FileSize, ma.AttachmentType, 0, createdUserId, TimeZoneId);
                                Count++;
                            } catch (Exception ex) {
                                throw ex;
                            }
        ...
        public async Task CreateMessageAttachment(int messageId, int assetId, int companyId, int createdUserId, string timeZoneId) {
            try {
                ...
                try {
                    var asset = await db.Set<Core.Assets.Assets>().Where(A => A.AssetId == assetId).FirstOrDefaultAsync();
                    if (asset != null) {
                        FileHandler FH = new FileHandler(db, _httpContextAccessor);
                        ...
                        if (HostingEnvironment.ToUpper() == "AZURE") {
                            ...
                            _ = FH.CopyFileAsync(AzurePortalShare, file_url, AzureAPIShare, @DirectoryPath + asset.AssetPath);

                            _ = CreateMediaAttachment(messageId, asset.AssetTitle, asset.AssetPath, asset.SourceFileName, (decimal)asset.AssetSize, 2, 0, createdUserId, timeZoneId);
                            Count++;
                        } else {
                            _DBC.connectUNCPath();
                            _DBC.connectUNCPath(AttachmentSavePath, AttachmentUncUser, AttachmentUncPwd, AttachmentUseUnc);
                    ...
                    if (Count > 0) {
                        var msg =  db.Set<Message>().Where(M => M.MessageId == messageId).FirstOrDefault();
                        if (msg != null) {
                            msg.AttachmentCount += Count;
                            await db.SaveChangesAsync();
                        }
        ...
        public async Task AddUserLocation(int userID, int userDeviceID, double latitude, double longitude, string locationAddress,
            ...
            try {
                if (latitude != 0 && longitude != 0) {
                    ...
                    _ = DBC.UpdateUserLocation(userID, companyID, latitude.ToString(), longitude.ToString(), TimeZoneId);
                }
        ...
        public void AddTrackMeUsers(int incidentActivationId, int messageId, int companyId) {
            try {
                ...
                if (UsersToNotify.Count > 0) {
                    AddUserToNotify(messageId, UsersToNotify).ConfigureAwait(true);
                }
             ...
             on I.ISDCode equals P.ISDCode
                ...
                if (pricing.Count > 0) {
                    ...
                    if ((cpp.CreditBalance + cpp.CreditLimit) < TotalPrice && ShouldCheckCost == true) {
                        HandleMessageMethods(messageId);
                        return false;
                    }
                }
        ...
        public void HandleMessageMethods(int MessageID) {

            try {
                var pMessageID = new SqlParameter("@MessageID", MessageID);
                db.Database.ExecuteSqlRaw("Pro_Fix_Message_Methods @MessageID", pMessageID);
        ...
        public async Task SocialPosting(int messageId, List<string> socialHandle, int companyId) {
            try {
                ...
                var msg = await (from M in db.Set<Message>() where M.MessageId == messageId select M).FirstOrDefaultAsync();
                ...
                if (msg != null) {
                    foreach (string Handle in socialHandle) {
                        ...
                        if (handle != null) {
                            if (Handle == "TWITTER") {
                                 await TwitterPost(msg.MessageText, handle.AdnlKeyOne, handle.AdnlKeyTwo, handle.AuthToken, handle.AuthSecret);
                            } else if (Handle == "LINKEDIN") {
                                LinkedInPost(msg.MessageText, handle.AdnlKeyOne, handle.AdnlKeyTwo, handle.AuthToken, handle.AuthSecret);
                            } else if (Handle == "FACEBOOK") {
                                FacekbookPost(msg.MessageText, handle.AdnlKeyOne, handle.AdnlKeyTwo, handle.AuthToken, handle.AuthSecret);
                            }
                        }
                    }
                }
        ...
        public async Task TwitterPost(string messageText, string consumerKey, string consumerSecret, string authToken, string authSecret) {
            ...
            try {
                //success posting
                WebResponse objWebResponse = await objHttpWebRequest.GetResponseAsync();
